...
try {
	let repos = JSON.parse(request('GET', "https://api.github.com/orgs/DoubleDotLabs/repos?per_page=1000", {
		headers: { 
			"User-Agent": "DoubleDotLabs.github.io",
	...
	for (let i = 0; i < repos.length; i++) {
		if (!repos[i].fork && (!repos[i].description || !repos[i].description.startsWith("("))) {
			...
			if (repo.homepage) {				
				if (repo.homepage.includes("doubledotlabs.github.io")) {
					links += "  - name: Website\n"
						+ "    url: " + repo.homepage + "\n"
